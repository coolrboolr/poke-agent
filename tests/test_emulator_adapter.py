...
def test_read_frame(monkeypatch):
    ...
    monkeypatch.setattr(adapter_module, "subprocess", types.SimpleNamespace(Popen=DummyPopen))
...
def test_send_input_logs(monkeypatch):
    pressed = []
    monkeypatch.setattr(adapter_module, "subprocess", types.SimpleNamespace(Popen=DummyPopen, run=lambda args, check=False: pressed.append(args[2])))
    monkeypatch.setattr(adapter_module.ImageGrab, "grab", lambda: Image.new("RGB", (1, 1)))
